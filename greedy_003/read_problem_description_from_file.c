 ...
 */
void read_problem_description_from_file(const char *path,
  ...
  specific_problem->sets =
      (struct set *)calloc(number_of_sets, sizeof(struct set));
  for (int i = 0; i < number_of_sets; ++i) {
    specific_problem->sets[i].index_of_root_element = 0;
    specific_problem->sets[i].elements = (struct dynamic_array_element *)malloc(
        sizeof(struct dynamic_array_element));
    initialize_element(specific_problem->sets[i].elements);
  }

  // allocate and initialize element-value-table
  specific_problem->element_value_table =
      (struct dynamic_array_index_pair *)calloc(
          number_of_elements, sizeof(struct dynamic_array_index_pair));
  ...
  while ((character = fgetc(file)) != EOF) {
    ...
    if ((character == ' ') || (character == '\n')) {
      ...
      if (!terminatingCharacterReached) {
        if (isCost) {
          ...
          element_value = atoi(&(string[0]));
					if (lineIndex == 0) printf("%d ", element_value);
          ...
          (specific_problem->element_value_table[element_value])
              .add(element_value_table_entry,
                   &(specific_problem->element_value_table[element_value]));
          // add element to set
          ...
          ((specific_problem->sets[lineIndex]).elements)
              ->add(_element, specific_problem->sets[lineIndex].elements);
      ...
      if (character == '\n') {
        ...
        // init efficiency
        specific_problem->sets[lineIndex].efficiency =
            specific_problem->sets[lineIndex].cost /
            specific_problem->sets[lineIndex].number_of_elements;
        // mark last element of set
        specific_problem->sets[lineIndex]
            .elements->data[element_index - 1]
            .index_next = -1;
        ...
        if (specific_problem->sets[lineIndex].efficiency <
            specific_problem->minimum_efficiency) {
